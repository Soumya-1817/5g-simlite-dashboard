 ...
 * ④ Comparison mode — run any two schedulers side-by-side
 * ⑤ Enhanced CSV export — includes noise config + retransmission data
 * ⑥ NEW: Proportional Fair (PF) scheduling — real 5G standard
 * ⑦ NEW: Priority-based scheduling (QoS classes) — real 5G standard
 * ⑧ NEW: Simulation time limit — user sets max run duration
 * ⑨ NEW: SVG scanline animation on network topology canvas
 */
...
// QoS Priority classes (5G NR QCI equivalents): index → priority tier label + color
const QOS_TIERS = [
  { label:"QCI-1 GBR",  color:"#ff4444", weight:4 },  // UE-0: highest
  { label:"QCI-4 GBR",  color:"#ff8800", weight:3 },  // UE-1
  { label:"QCI-6 NGBR", color:"#ffcc00", weight:2 },  // UE-2
  { label:"QCI-8 NGBR", color:"#39ff14", weight:1 },  // UE-3
  { label:"QCI-9 NGBR", color:"#00f5ff", weight:1 },  // UE-4
  { label:"QCI-9 NGBR", color:"#a855f7", weight:1 },  // UE-5
];
...
const SCHED_DESC = {
  FIFO: "First In First Out — packets served in exact arrival order. Simple, no starvation, but unfair under load.",
  RR:   "Round Robin — each UE served in strict rotation. Guarantees equal share regardless of channel conditions.",
  EDF:  "Earliest Deadline First — soonest deadline always served first. Optimal for real-time traffic; can starve best-effort.",
  PF:   "Proportional Fair — balances throughput vs fairness. Selects UE with best ratio of instantaneous rate to average throughput. Used in real 5G NR schedulers.",
  PRIO: "Priority-Based (QoS Classes) — UEs assigned QCI tiers (GBR/NGBR). Higher-priority UEs always scheduled first. Mirrors 5G QoS Framework.",
};
...
/**
 * Proportional Fair — mirrors 5G NR PF scheduler.
 * Scores each UE as: instantaneous_rate / avg_throughput
 * instantaneous_rate proxy: 1000/size (smaller pkts → higher modulation)
 * avgTp: per-UE EWMA of delivered rate (maintained in sim.current.avgTp)
 */
const schedPF = (q, avgTp) => {
  if (!q.length) return null;
  let best = null, bestScore = -Infinity;
  // One candidate per UE (their head-of-line packet)
  const candidates = new Map();
  for (const p of q) {
    if (!candidates.has(p.ueId)) candidates.set(p.ueId, p);
  }
  for (const [ueId, p] of candidates) {
    const instRate = 1000 / Math.max(p.sz, 1);
    const avg = avgTp[ueId] ?? 0.01;
    const score = instRate / avg;
...
/**
 * Priority-Based (QoS) — mirrors 5G QCI framework.
 * UE index maps to QoS tier weight (higher index = lower priority).
 * Within same tier, FIFO.
 */
const schedPriority = (q, ues) => {
  if (!q.length) return null;
  // Build ueId → weight map from current UE list
  const weightOf = {};
  ues.forEach((ue, idx) => {
    weightOf[ue.id] = QOS_TIERS[idx % QOS_TIERS.length].weight;
  });
  return [...q].sort((a, b) => {
    const wa = weightOf[a.ueId] ?? 1;
    const wb = weightOf[b.ueId] ?? 1;
    if (wb !== wa) return wb - wa; // higher weight first
    return a.born - b.born;       // FIFO tiebreak
  })[0];
};
...
// CSV EXPORT
// ═══════════════════════════════════════════════════════════
function exportCSV({algo,rate,speed,noise,maxTime,numUes,history,events,metrics,label=""}) {
  ...
  lines.push(`# Simulation Time Limit,${maxTime>0?maxTime+"s":"unlimited"}`);
...
function useJSSim(initAlgo="FIFO", mini=false) {
  ...
  const [maxTime,   setMaxTime]   = useState(0);      // 0 = unlimited
  ...
  const [completed, setCompleted] = useState(false);  // finished by time limit

  const sim    = useRef({t:0,q:[],rrPtr:0,nxtMap:{0:0,1:0,2:0},txing:false,
                         sent:0,drop:0,retx:0,dlys:[],n:0,gen:0,avgTp:{}});
  ...
  const doTick = () => {
    const s=sim.current, gen=s.gen, curUes=uesRef.current;
    const sp=spRef.current, r=rtRef.current, nr=noiseRef.current, mt=maxTimeRef.current;
    s.t += BASE_DT*sp;

    // ── Simulation time limit check ───────────────────────
    if (mt > 0 && s.t >= mt) {
      clearInterval(iRef.current);
      setStatus("idle");
      setCompleted(true);
      logEv(s.t, `✓ Simulation completed — time limit ${mt}s reached`, "info");
    ...
    curUes.forEach((ue, idx) => {
      ...
      if (s.t >= s.nxtMap[ue.id]) {
        ...
        const p = {id:++s.n, ueId:ue.id, color:ue.color, name:ue.name,
                   born:s.t, dl:s.t+1.5+Math.random()*3.5, sz:100+~~(Math.random()*450)};
        ...
        setTimeout(()=>{
          ...
          if (s.q.length>=QUEUE_CAP) {
            s.drop++; setMetrics(m=>({...m,drops:s.drop}));
            logEv(s.t, `PKT #${p.id} DROPPED — queue full!`, "drop");
          } else {
            s.q.push(p); setQueue([...s.q]);
            logEv(s.t, `PKT #${p.id} queued [${s.q.length}/${QUEUE_CAP}]`, "queue");
          }
        }, animMs+30);
      }
    });
    ...
    if (!s.txing && s.q.length > 0) {
      ...
      let sel;
      if      (al==="FIFO") sel = schedFIFO(s.q);
      else if (al==="RR")   { sel = schedRR(s.q, s.rrPtr); s.rrPtr++; }
      else if (al==="EDF")  sel = schedEDF(s.q);
      else if (al==="PF")   sel = schedPF(s.q, s.avgTp);
      else if (al==="PRIO") sel = schedPriority(s.q, uesRef.current);
      else                  sel = s.q[0];
      if (!sel) sel = s.q[0];
      ...
      setTimeout(()=>{
        ...
        if (Math.random() < nr) {
          ...
          // ── PF: update per-UE EWMA throughput ──────────────
          const alpha = 0.1;
          const instRate = 1000 / Math.max(sel.sz, 1);
          s.avgTp[sel.ueId] = s.avgTp[sel.ueId]
            ? (1-alpha)*s.avgTp[sel.ueId] + alpha*instRate
            : instRate;
  ...
  const start = () => {
    ...
    if (maxTimeRef.current > 0) logEv(0, `⏱ Time limit: ${maxTimeRef.current}s`, "info");
  };
  const pause  = () => { clearInterval(iRef.current); setStatus("paused"); logEv(sim.current.t,"⏸ Paused","info"); };
  const resume = () => { setStatus("running"); iRef.current=setInterval(()=>tkRef.current(),TICK_MS); logEv(sim.current.t,"▶ Resumed","info"); };
  const reset  = () => {
    clearInterval(iRef.current);
    const nm={}; uesRef.current.forEach(u=>{nm[u.id]=0;});
    sim.current={t:0,q:[],rrPtr:0,nxtMap:nm,txing:false,sent:0,drop:0,retx:0,dlys:[],n:0,gen:sim.current.gen+1,avgTp:{}};
    evRef.current=[]; anRef.current=[];
    setStatus("idle"); setCompleted(false); setQueue([]); setEvents([]); setAnims([]); setTxPkt(null);
  ...
  return {
    ...
    queue, events, metrics, txPkt, anims, history, activeUes, rxFlash, simT, completed,
...
function HomePage({ onCompare }) {
  ...
  const [maxTimeInput, setMaxTimeInput] = useState("0");

  const randomizeNoise = () => {
    const r = +(Math.random()*0.40).toFixed(2);
    setNoise(r); setNoiseInput((r*100).toFixed(0));
  };
  const handleNoiseSlider = (v) => { setNoise(v); setNoiseInput((v*100).toFixed(0)); };
  const handleNoiseInput  = (v) => {
    const n = Math.min(50,Math.max(0,+v||0));
    setNoiseInput(String(n)); setNoise(+(n/100).toFixed(3));
  };
  const handleMaxTimeInput = (v) => {
    const n = Math.max(0, +v||0);
    setMaxTimeInput(String(n)); setMaxTime(n);
  };
  ...
  const handleWsMsg = useCallback((raw)=>{
    ...
    switch(msg.type){
      case "gen":{
        ...
        setTimeout(()=>sim.setActiveUesExt(prev=>{const n=new Set(prev);n.delete(msg.ueId);return n;}),450);
        logPy(t,`PKT #${msg.pktId} generated at ${msg.ueName} (${msg.sz}B)`,"gen");
        break;
      }
      case "drop":   logPy(t,msg.msg,"drop"); sim.setMetricsExt(m=>({...m,drops:msg.drops})); break;
      case "queued": sim.setQueueExt(msg.q??[]); logPy(t,msg.msg,"queue"); break;
      case "retx":   logPy(t,msg.msg,"retx"); sim.setMetricsExt(m=>({...m,retx:msg.retx})); break;
      case "sched":{
      ...
      case "rx":
        ...
        if(msg.metrics){
          ...
          sim.setHistoryExt(h=>[...h.slice(-60),{t:~~(+t),tp:msg.metrics.tp,d:msg.metrics.delay}]);
        }
        break;
      case "done":
        setPyStatus("idle");
        logPy(t, msg.msg ?? "✓ Simulation time limit reached", "info");
        break;
      case "info": logPy(t,msg.msg,"info"); break;
      default: break;
    }
  },[logPy,sim]);
  ...
  const connectWS = useCallback(()=>{
    ...
    ws.onclose = ()=>{ setWsState("closed"); setPyStatus("idle"); logPy(0,"🔌 Disconnected","info"); };
    ws.onerror = ()=>{ setWsState("error");  logPy(0,"⚠ WebSocket error — is backend running?","drop"); };
  ...
  const statusColor  = completed ? "#a855f7" : {idle:"#ff4444",running:"#39ff14",paused:"#ffcc00"}[activeStatus];
  ...
  const doStart = ()=>{
    ...
    if(isPython){
      if(wsRef.current?.readyState!==WebSocket.OPEN){
        logPy(0,"⚠ Backend not connected","drop"); return;
      }
      setPyStatus("running");
      wsRef.current.send(JSON.stringify({
        type:"start", algo, rate, numUes:ues.length, speed, noiseRate:noise, maxTime,
      }));
  ...
  // Priority tier info for current UEs
  const getPriorityLabel = (idx) => QOS_TIERS[idx % QOS_TIERS.length].label;
  const getPriorityColor = (idx) => QOS_TIERS[idx % QOS_TIERS.length].color;
    ...
    <div style={{fontFamily:"'Share Tech Mono',monospace",background:"#030d1a",minHeight:"100vh",color:"#b8ccd8",overflow:"hidden"}}>
      ...
      <header className="app-header">
        <div>
          ...
          <div className="brand-sub">PACKET SCHEDULER SIMULATION v5</div>
        </div>
        <div className="header-controls">
          ...
          <div style={{textAlign:"right"}}>
            ...
            <div style={{fontFamily:"Orbitron",fontSize:14,color: completed?"#a855f7":"#ffcc00"}}>
              {(isPython ? pySimT : simT).toFixed(2)}s
              {maxTime>0 && <span style={{fontSize:8,color:"#3a4a5a",marginLeft:4}}>/ {maxTime}s</span>}
            </div>
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{fontSize:8,color:statusColor,letterSpacing:3}}>
              {completed ? "DONE" : activeStatus.toUpperCase()}
            </span>
          </div>
          ...
          <button onClick={onCompare} className="compare-btn" title="Comparison mode">⚡ COMPARE</button>
          <button
            onClick={()=>exportCSV({algo,rate,speed,noise,maxTime,numUes:ues.length,history,events,metrics})}
            disabled={!canExport}
      ...
      {/* ═══ COMPLETED BANNER ════════════════════════════ */}
      {completed && (
        <div style={{textAlign:"center",padding:"5px",background:"rgba(168,85,247,.08)",borderBottom:"1px solid rgba(168,85,247,.25)"}}>
          <span style={{fontFamily:"Orbitron",fontSize:11,color:"#a855f7",letterSpacing:3}}>
            ✓ SIMULATION COMPLETE — {simT.toFixed(2)}s elapsed
          </span>
          <span style={{fontSize:9,color:"#4a2a6a",marginLeft:10}}>Press RESET to run again</span>
        </div>
      )}
      ...
      <div className="main-grid">
        ...
        <div className="left-panel">
          ...
          <div className="panel" style={{padding:12}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
              {activeStatus==="idle"    && !completed && <Btn color="#39ff14" onClick={doStart} disabled={isPython&&wsState!=="open"} key="btn-start">▶ START</Btn>}
            ...
            <div style={{marginBottom:12,padding:"8px 10px",background:"rgba(168,85,247,.06)",border:"1px solid rgba(168,85,247,.2)",borderRadius:4}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
                <span style={{fontSize:9,color:"#a855f7",letterSpacing:2}}>⏱ SIM TIME LIMIT</span>
                <span style={{fontFamily:"Orbitron",fontSize:14,color:maxTime>0?"#a855f7":"#2a3a4a"}}>
                  {maxTime>0?`${maxTime}s`:"∞"}
                </span>
              </div>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                <input type="range" min="0" max="120" step="5" value={maxTime}
                  onChange={e=>{setMaxTime(+e.target.value);setMaxTimeInput(String(+e.target.value));}}
                  style={{flex:1,accentColor:"#a855f7"}}/>
                <input type="number" min="0" max="9999" value={maxTimeInput}
                  onChange={e=>handleMaxTimeInput(e.target.value)}
                  style={{width:52,padding:"3px 6px",background:"rgba(168,85,247,.08)",
                    border:"1px solid rgba(168,85,247,.3)",borderRadius:3,
                    color:"#a855f7",fontSize:11,fontFamily:"Share Tech Mono",textAlign:"center"}}/>
                <span style={{fontSize:9,color:"#3a4a5a"}}>s</span>
              </div>
              <div style={{display:"flex",gap:4,marginTop:6,flexWrap:"wrap"}}>
                {[0,10,30,60,120].map(t=>(
                  <button key={t} onClick={()=>{setMaxTime(t);setMaxTimeInput(String(t));}}
                    style={{padding:"2px 7px",background:maxTime===t?"rgba(168,85,247,.18)":"rgba(168,85,247,.04)",
                      border:`1px solid ${maxTime===t?"rgba(168,85,247,.5)":"rgba(168,85,247,.12)"}`,
                      color:maxTime===t?"#a855f7":"#3a4a5a",borderRadius:3,fontSize:9,fontFamily:"Share Tech Mono",cursor:"pointer"}}>
                    {t===0?"∞":`${t}s`}
                  </button>
                ))}
              </div>
              <div style={{fontSize:7,color:"#3a4a5a",marginTop:5}}>
                {maxTime===0?"Run indefinitely until RESET.":
                 `Auto-stop after ${maxTime}s of simulated time.`}
              </div>
            </div>

            {/* Scheduler */}
            <SL style={{marginBottom:7}}>SCHEDULER ALGORITHM</SL>
            {["FIFO","RR","EDF","PF","PRIO"].map(a=>(
              <button key={a}
                ...
                {a==="FIFO"?"First In First Out":
                 ...
                 a==="PF"?"Proportional Fair":"Priority-Based (QoS)"}
                {(a==="PF"||a==="PRIO")&&(
                  <span style={{marginLeft:6,padding:"1px 5px",background:"rgba(57,255,20,.12)",
                    border:"1px solid rgba(57,255,20,.3)",borderRadius:2,fontSize:7,color:"#39ff14",verticalAlign:"middle"}}>
                    5G NR
                  </span>
                )}
              </button>
            ...
            <div style={{marginBottom:8,padding:"6px 9px",background:"rgba(255,136,0,.07)",border:"1px solid rgba(255,136,0,.2)",borderRadius:4}}>
              ...
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                ...
                <button onClick={randomizeNoise} style={{padding:"4px 9px",background:"rgba(168,85,247,.1)",
                  ...
                  fontFamily:"Share Tech Mono",letterSpacing:1}}>🎲 RANDOM</button>
              </div>
          ...
          <div className="panel" style={{padding:12}}>
            ...
            {ues.map((ue,idx)=>{
              ...
              const pColor = algo==="PRIO" ? getPriorityColor(idx) : ue.color;
              return(
                <div key={ue.id} className="ue-enter" style={{display:"flex",alignItems:"center",gap:7,marginBottom:7,padding:"6px 8px",background:act?`${ue.color}0c`:"transparent",border:`1px solid ${act?ue.color+"38":"transparent"}`,borderRadius:4,transition:"all .3s"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    {algo==="PRIO"
                      ? <div style={{fontSize:7,color:pColor}}>{getPriorityLabel(idx)}</div>
                  ...
                  <button onClick={()=>removeUE(ue.id)} disabled={ues.length<=1||activeStatus==="running"}
                    style={{width:15,height:15,borderRadius:"50%",background:"rgba(255,68,68,.08)",border:"1px solid rgba(255,68,68,.2)",color:"#ff4444",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",padding:0,opacity:(ues.length<=1||activeStatus==="running")?.2:.65,flexShrink:0}}>×</button>
                </div>
              );
            })}
        ...
        <div className="center-panel">
          <div className="panel grid-bg" style={{flex:"1 1 0",position:"relative",overflow:"hidden",minHeight:0}}>
            ...
            <div style={{position:"absolute",top:8,right:12,fontSize:7,zIndex:2}}>
              {isPython
                ...
                : <><span style={{color:"rgba(255,204,0,.3)"}}>SPEED </span><span style={{color:"#ffcc00"}}>{speed}×</span>
                   ...
                   {maxTime>0&&<span style={{marginLeft:8,color:"#a855f7"}}>⏱{maxTime}s</span>}</>
              }
            </div>
          ...
          <div className="panel" style={{padding:12,flexShrink:0}}>
            ...
            <div style={{display:"flex",gap:5,flexWrap:"wrap",minHeight:46}}>
              {Array.from({length:QUEUE_CAP}).map((_,i)=>{
                ...
                const urgent=p&&algo==="EDF"&&((p.dl??99)-simT)<1;
                const ueIdx = p ? ues.findIndex(u=>u.id===p.ueId) : -1;
                const pColor = p && algo==="PRIO" && ueIdx>=0 ? getPriorityColor(ueIdx) : c;
                return(
                  <div key={i} title={p?`PKT#${p.id}|${p.name}|${p.sz}B`:"empty"}
                    style={{width:59,height:46,borderRadius:4,flexShrink:0,
                      border:`1px solid ${p?(urgent?"#ff4444":pColor):"rgba(0,245,255,.05)"}`,
                      background:p?(urgent?"#ff444413":`${pColor}11`):"rgba(0,245,255,.012)",
                      display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
                      fontSize:8,color:p?pColor:"#1a2a38",transition:"all .25s"}}>
                    {p?(<>
                      ...
                      {algo==="PRIO"&&ueIdx>=0&&<div style={{fontSize:6,color:pColor}}>W{QOS_TIERS[ueIdx%QOS_TIERS.length].weight}</div>}
        ...
        <div className="right-panel">
          <div className="panel" style={{padding:12,flexShrink:0}}>
            ...
            <MC label="RETRANSMISSIONS"  value={metrics.retx??0} unit=""   color="#ff8800" tip="Packets re-sent due to channel noise."       onHover={setHovTip}/>
          </div>
...
function ComparePage({ onBack }) {
  ...
  const [sharedMaxTime, setSharedMaxTime] = useState(0);
  const [noiseInput,    setNoiseInput]    = useState("0");
  const [maxTimeInput,  setMaxTimeInput]  = useState("0");

  const simA = useJSSim("FIFO", true);
  const simB = useJSSim("PF",   true);
  ...
  useEffect(()=>{ simA.setMaxTime(sharedMaxTime); simB.setMaxTime(sharedMaxTime); },[sharedMaxTime]);
  ...
  const bothDone    = simA.completed&&simB.completed;
  ...
  const winner = (()=>{
    ...
    const scoreA=tA-dA*2, scoreB=tB-dB*2;
  ...
  return (
    <div style={{fontFamily:"'Share Tech Mono',monospace",background:"#030d1a",minHeight:"100vh",color:"#b8ccd8",overflow:"auto"}}>
      ...
      <header className="app-header" style={{flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
          ...
          <div>
            ...
            <input type="range" min="0.5" max="7" step="0.5" value={sharedRate} onChange={e=>setSharedRate(+e.target.value)} style={{width:90,accentColor:"#00f5ff"}}/>
          </div>
          {/* Speed */}
          <div>
            <div className="ctrl-label">SPEED</div>
            <div style={{display:"flex",gap:3}}>
              {SPEED_OPTS.map(s=>(
                <button key={s} className={`spd${sharedSpeed===s?" on":""}`} style={{width:30,fontSize:9}} onClick={()=>setSharedSpeed(s)}>{s}×</button>
              ))}
            </div>
          </div>
          {/* Noise */}
          ...
          {/* Max time */}
          <div>
            <div className="ctrl-label">TIME LIMIT: <span style={{color:"#a855f7"}}>{sharedMaxTime>0?`${sharedMaxTime}s`:"∞"}</span></div>
            <div style={{display:"flex",gap:4,alignItems:"center"}}>
              <input type="range" min="0" max="120" step="5" value={sharedMaxTime}
                onChange={e=>{setSharedMaxTime(+e.target.value);setMaxTimeInput(String(+e.target.value));}}
                style={{width:70,accentColor:"#a855f7"}}/>
              <input type="number" min="0" value={maxTimeInput}
                onChange={e=>{const n=Math.max(0,+e.target.value||0);setMaxTimeInput(String(n));setSharedMaxTime(n);}}
                style={{width:42,padding:"2px 5px",background:"rgba(168,85,247,.08)",border:"1px solid rgba(168,85,247,.3)",borderRadius:3,color:"#a855f7",fontSize:10,fontFamily:"Share Tech Mono",textAlign:"center"}}/>
              <span style={{fontSize:8,color:"#3a4a5a"}}>s</span>
            </div>
          </div>
          {/* UEs */}
          <div>
            ...
            <div style={{display:"flex",gap:4}}>
              <button onClick={()=>handleUEChange("add")} disabled={simA.ues.length>=MAX_UES||!bothIdle}
                style={{padding:"3px 9px",background:"rgba(57,255,20,.1)",border:"1px solid rgba(57,255,20,.3)",color:"#39ff14",borderRadius:3,fontSize:13,fontWeight:"bold",opacity:(simA.ues.length>=MAX_UES||!bothIdle)?.3:1}}>+</button>
              <button onClick={()=>handleUEChange("remove")} disabled={simA.ues.length<=1||!bothIdle}
                style={{padding:"3px 9px",background:"rgba(255,68,68,.1)",border:"1px solid rgba(255,68,68,.3)",color:"#ff4444",borderRadius:3,fontSize:13,fontWeight:"bold",opacity:(simA.ues.length<=1||!bothIdle)?.3:1}}>−</button>
            </div>
          </div>
          {/* Controls */}
          ...
          {(simA.history.length>0||simB.history.length>0)&&(
            <div style={{display:"flex",gap:4}}>
              <button onClick={()=>exportCSV({algo:simA.algo,rate:sharedRate,speed:sharedSpeed,noise:sharedNoise,maxTime:sharedMaxTime,numUes:simA.ues.length,history:simA.history,events:simA.events,metrics:simA.metrics,label:"A"})}
              ...
              <button onClick={()=>exportCSV({algo:simB.algo,rate:sharedRate,speed:sharedSpeed,noise:sharedNoise,maxTime:sharedMaxTime,numUes:simB.ues.length,history:simB.history,events:simB.events,metrics:simB.metrics,label:"B"})}
      ...
      {bothDone&&(simA.metrics.tx+simB.metrics.tx)>0&&(
        <div style={{textAlign:"center",padding:"5px",background:"rgba(168,85,247,.07)",borderBottom:"1px solid rgba(168,85,247,.2)"}}>
          <span style={{fontFamily:"Orbitron",fontSize:11,color:"#a855f7",letterSpacing:3}}>✓ BOTH SIMULATIONS COMPLETE</span>
        </div>
      )}
      {winner&&winner!=="tie"&&!bothDone&&(simA.metrics.tx+simB.metrics.tx)>5&&(
        <div style={{textAlign:"center",padding:"6px",background:"rgba(57,255,20,.07)",borderBottom:"1px solid rgba(57,255,20,.15)"}}>
          <span style={{fontFamily:"Orbitron",fontSize:11,color:"#39ff14",letterSpacing:3}}>★ WINNER: SCHEDULER {winner} — {winner==="A"?simA.algo:simB.algo}</span>
        </div>
      )}
      ...
      {(simA.metrics.tx+simB.metrics.tx)>2&&(
        <div className="panel" style={{margin:"0 8px 8px",padding:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            {[
              {key:"tp",  label:"THROUGHPUT (pkt/s) — higher is better", aVal:simA.metrics.tp,    bVal:simB.metrics.tp},
              {key:"dl",  label:"AVG DELAY (s) — lower is better",       aVal:simA.metrics.delay, bVal:simB.metrics.delay},
              {key:"dr",  label:"PKT DROPS — lower is better",            aVal:simA.metrics.drops, bVal:simB.metrics.drops},
              {key:"rx",  label:"RETRANSMISSIONS — lower is better",      aVal:simA.metrics.retx??0, bVal:simB.metrics.retx??0},
            ].map(({key,label,aVal,bVal})=>(
              <div key={key}>
                <div style={{fontSize:8,color:"#2a3a4a",letterSpacing:2,marginBottom:6}}>{label}</div>
                <ResponsiveContainer width="100%" height={100}>
                  <BarChart data={[{name:key,A:aVal,B:bVal}]} margin={{top:4,right:4,bottom:0,left:0}}>
...
function MiniSimPanel({ sim, label, labelColor }) {
  const { status,algo,setAlgo,queue,events,metrics,txPkt,anims,history,activeUes,rxFlash,simT,ues,completed } = sim;
  const queueFull = queue.length>=QUEUE_CAP;
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",gap:7,padding:8,minWidth:0}}>
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",background:"rgba(2,16,36,.7)",border:`1px solid ${labelColor}22`,borderRadius:6}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{display:"flex",gap:3,marginTop:2,flexWrap:"wrap"}}>
            {["FIFO","RR","EDF","PF","PRIO"].map(a=>(
              <button key={a} onClick={()=>setAlgo(a)} style={{padding:"2px 6px",background:algo===a?`${labelColor}18`:"transparent",border:`1px solid ${algo===a?labelColor+"44":labelColor+"18"}`,color:algo===a?labelColor:labelColor+"44",borderRadius:3,fontSize:8,fontFamily:"Share Tech Mono",cursor:"pointer"}}>
                {a}
              </button>
            ))}
          </div>
        ...
        <div style={{textAlign:"right"}}>
          ...
          <div style={{fontFamily:"Orbitron",fontSize:12,color:completed?"#a855f7":"#ffcc00"}}>{simT.toFixed(1)}s</div>
          {completed&&<div style={{fontSize:7,color:"#a855f7"}}>DONE</div>}
        </div>
      ...
      <div className="panel" style={{padding:8,flexShrink:0}}>
        ...
        {txPkt&&<div style={{marginTop:5,fontSize:7,color:txPkt.color}}>◆ TX: PKT #{txPkt.id} | {txPkt.name}</div>}
      </div>
...
// ═══════════════════════════════════════════════════════════
// NET SVG — with scanline animation
// ═══════════════════════════════════════════════════════════
function NetSVG({ txPkt, anims, activeUes, rxFlash, algo, ues, uePosArr, bsNode, rxNode, svgW, svgH }) {
  // Unique IDs to avoid gradient conflicts between mini and full SVGs
  const uid = svgW > 500 ? "full" : "mini";

  return (
    <svg viewBox={`0 0 ${svgW} ${svgH}`} style={{width:"100%",height:"100%"}}>
      <defs>
        <filter id={`gCy${uid}`}><feGaussianBlur stdDeviation="4" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        <filter id={`gAm${uid}`}><feGaussianBlur stdDeviation="5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        <filter id={`gSo${uid}`}><feGaussianBlur stdDeviation="3" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>

        {/* Scanline gradient */}
        <linearGradient id={`scanGrad${uid}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stopColor="#00f5ff" stopOpacity="0"/>
          <stop offset="40%"  stopColor="#00f5ff" stopOpacity="0.04"/>
          <stop offset="50%"  stopColor="#00f5ff" stopOpacity="0.13"/>
          <stop offset="60%"  stopColor="#00f5ff" stopOpacity="0.04"/>
          <stop offset="100%" stopColor="#00f5ff" stopOpacity="0"/>
        </linearGradient>

        {/* Horizontal bright line gradient */}
        <linearGradient id={`scanLine${uid}`} x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%"   stopColor="#00f5ff" stopOpacity="0"/>
          <stop offset="20%"  stopColor="#00f5ff" stopOpacity="0.15"/>
      ...
      <g>
        {/* Soft glow band */}
        <rect x={0} y={0} width={svgW} height={svgH*0.18} fill={`url(#scanGrad${uid})`}>
          <animateTransform attributeName="transform" type="translate"
            from={`0,${-svgH*0.18}`} to={`0,${svgH+svgH*0.18}`}
            dur="4s" repeatCount="indefinite"/>
        </rect>
        {/* Sharp bright leading edge */}
        <rect x={0} y={0} width={svgW} height={2} fill={`url(#scanLine${uid})`} opacity="0.7">
          <animateTransform attributeName="transform" type="translate"
            from={`0,${-2}`} to={`0,${svgH+2}`}
            dur="4s" repeatCount="indefinite"/>
        </rect>
      </g>
      ...
      {ues.map((ue,idx)=>{
        ...
        return(
          <g key={ue.id} filter={`url(#gSo${uid})`}>
            {act&&(
            ...
            {/* QoS label for priority mode */}
            {algo==="PRIO"&&(
              <text x={uep.x+r+4} y={uep.y-4} textAnchor="start" fill={QOS_TIERS[idx%QOS_TIERS.length].color}
                fontSize={5} fontFamily="Share Tech Mono" opacity={.8}>
                {QOS_TIERS[idx%QOS_TIERS.length].label}
              </text>
            )}
      ...
      {/* Receiver */}
      <g className={rxFlash?"rx-lit":""} filter={`url(#gAm${uid})`}>
  ...
  return(
    <style>{`
      ...
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      .spd{padding:3px 0;border-radius:3px;font-size:9px;font-family:'Share Tech Mono',monospace;border:1px solid rgba(0,245,255,.13);background:rgba(0,245,255,.03);color:#3a5a6a;cursor:pointer;transition:all .13s;text-align:center;}
      ...
      .mode-btn{padding:4px 11px;border-radius:3px;font-size:9px;font-family:'Share Tech Mono',monospace;letter-spacing:1px;cursor:pointer;transition:all .13s;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.07);color:#2a3a4a;}
      ...
      .compare-btn{padding:5px 11px;background:rgba(168,85,247,.1);border:1px solid rgba(168,85,247,.35);color:#a855f7;border-radius:4px;font-size:10px;font-family:'Share Tech Mono',monospace;letter-spacing:1px;}
      .csv-btn{padding:5px 11px;background:rgba(57,255,20,.1);border:1px solid rgba(57,255,20,.35);color:#39ff14;border-radius:4px;font-size:10px;font-family:'Share Tech Mono',monospace;letter-spacing:1px;}
      ...
      .app-header{padding:8px 14px;border-bottom:1px solid rgba(0,245,255,.1);display:flex;align-items:center;justify-content:space-between;background:rgba(0,245,255,.01);gap:10px;}
      .header-controls{display:flex;align-items:center;gap:12px;flex-wrap:wrap}

      .main-grid{display:grid;grid-template-columns:252px 1fr 268px;gap:8px;padding:8px;height:calc(100vh - 50px);}
      ...
      @media (max-width:1100px){.main-grid{grid-template-columns:220px 1fr 240px}}
      @media (max-width:880px){
        .main-grid{grid-template-columns:1fr 1fr;grid-template-rows:auto auto;height:auto;overflow:auto;}
...
function Btn({ children, color, onClick, disabled=false }) {
  return(
    <button onClick={onClick} disabled={disabled} style={{
      padding:"5px 12px",background:disabled?`${color}06`:`${color}15`,
      border:`1px solid ${disabled?color+"15":color+"55"}`,color:disabled?color+"30":color,
      borderRadius:4,fontSize:10,fontFamily:"Share Tech Mono,monospace",letterSpacing:1,
